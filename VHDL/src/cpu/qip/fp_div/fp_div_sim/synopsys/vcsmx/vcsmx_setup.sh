...
TOP_LEVEL_NAME="fp_div"
QSYS_SIMDIR="./../../"
QUARTUS_INSTALL_DIR="/opt/Quartus_17/quartus/"
SKIP_FILE_COPY=0
SKIP_DEV_COM=0
SKIP_COM=0
SKIP_ELAB=0
...
if [ $SKIP_FILE_COPY -eq 0 ]; then
  cp -f $QSYS_SIMDIR/fp_div_memoryC0_uid113_invTables_lutmem.hex ./
  cp -f $QSYS_SIMDIR/fp_div_memoryC1_uid116_invTables_lutmem.hex ./
  cp -f $QSYS_SIMDIR/fp_div_memoryC2_uid120_invTables_lutmem.hex ./
  cp -f $QSYS_SIMDIR/fp_div_memoryC1_uid117_invTables_lutmem.hex ./
  cp -f $QSYS_SIMDIR/fp_div_memoryC0_uid112_invTables_lutmem.hex ./
fi
...
if [ $SKIP_DEV_COM -eq 0 ]; then
  ...
  vlogan +v2k $USER_DEFINED_COMPILE_OPTIONS           "$QUARTUS_INSTALL_DIR/eda/sim_lib/synopsys/fiftyfivenm_atoms_ncrypt.v" -work fiftyfivenm 
...
if [ $SKIP_COM -eq 0 ]; then
  ...
  vhdlan -xlrm $USER_DEFINED_COMPILE_OPTIONS "$QSYS_SIMDIR/fp_div.vhd"               
fi
